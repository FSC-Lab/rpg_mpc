...
#define GET_PARAM_(name) \
  if (!quadrotor_common::getParam(#name, name##_, pnh)) return false
    ...
    Q_ = (Eigen::Matrix<T, kCostSize, 1>() << Q_pos_xy,
          Q_pos_xy,
          Q_pos_z,
          Q_attitude,
          Q_attitude,
          ...
          Q_perception)
             .finished()
             .asDiagonal();
    R_ = (Eigen::Matrix<T, kInputSize, 1>() << R_thrust,
          R_pitchroll,
          R_pitchroll,
          R_yaw)
             .finished()
    ...
    quadrotor_common::getParam(
        "state_cost_exponential", state_cost_exponential_, (T)0.0, pnh);
    quadrotor_common::getParam(
        "input_cost_exponential", input_cost_exponential_, (T)0.0, pnh);
