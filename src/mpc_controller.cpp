...
MpcController<T>::MpcController(const ros::NodeHandle& nh,
  ...
  sub_point_of_interest_ =
      nh_.subscribe("mpc/point_of_interest",
                    1,
                    &MpcController<T>::pointOfInterestCallback,
                    this);
  sub_autopilot_off_ =
...
quadrotor_common::ControlCommand MpcController<T>::run(
  ...
  return updateControlCommand(
      predicted_states_.col(0), predicted_inputs_.col(0), call_time);
}
...
bool MpcController<T>::setNewParams(MpcParams<T>& params) {
  ...
  mpc_wrapper_.setLimits(params.min_thrust_,
                         params.max_thrust_,
                         params.max_bodyrate_xy_,
